...
export const LineModifier: React.FC = () => {
  ...
  const [modifyMode, setModifyMode] = useState<'delete' | 'replace'>('delete');
  const [lineType, setLineType] = useState<'first' | 'last' | 'range'>('first');
  const [lineRange, setLineRange] = useState<string>('');
  const [replacementText, setReplacementText] = useState<string>('');
  const [replaceType, setReplaceType] = useState<'text' | 'file'>('text');
  ...
  const [totalModifiedLines, setTotalModifiedLines] = useState(0);
  ...
  const actualReplacementTextRef = useRef('');
  ...
  const handleSelectSourceFile = async () => {
    try {
      ...
      const paths = await window.electron.selectFiles({
        properties: ['openFile'],
        filters: [
          { name: t('文本文件'), extensions: ['txt', 'csv', 'json', 'md', 'log', 'sql', 'html', 'htm', 'xml'] },
  ...
  const handlePreviewReplacement = async () => {
    try {
      let content = replacementText;
      if (modifyMode === 'replace' && replaceType === 'file') {
        if (!sourceFilePath) {
          message.warning(t('请先选择包含替换内容的源文件'));
          return;
        }
        // @ts-ignore
        content = await window.electron.readFile(sourceFilePath);
      }
      setPreviewContent(content || '');
      setIsPreviewVisible(true);
    } catch (err) {
      message.error(t('预览替代内容失败'));
    }
  };
  ...
  const startProcessing = async () => {
    requireAuth(async () => {
      ...
      if (lineType === 'range' && !lineRange.trim()) {
        message.error(t('请输入行范围'));
        return;
      }

      const hasActive = fileList.some(f => 
        f.status === 'pending' || f.status === 'processing' || f.status === 'paused'
      );
      let listForRun = fileList;
      if (!hasActive && fileList.length > 0) {
        listForRun = fileList.map(f => ({
          ...f,
          status: 'pending',
        ...
        setFileList(listForRun);
        setProcessedCount(0);
        setTotalModifiedLines(0);
      }

      let actualText = replacementText;
      if (modifyMode === 'replace' && replaceType === 'file') {
        if (!sourceFilePath) {
          message.error(t('请选择包含替换内容的源文件'));
          setIsProcessing(false);
          isProcessingRef.current = false;
          return;
        }
        try {
          // @ts-ignore
          actualText = await window.electron.readFile(sourceFilePath);
        } catch (err) {
          message.error(t('读取源文件失败'));
          setIsProcessing(false);
          isProcessingRef.current = false;
          return;
        }
      }
      actualReplacementTextRef.current = actualText;
      ...
      processQueue(listForRun);
    });
  };

  const processQueue = async (listParam?: FileItem[]) => {
    const sourceList = listParam || fileList;
    const pendingFiles = sourceList.filter(f => f.status === 'pending' || f.status === 'paused');
    ...
    for (const file of pendingFiles) {
      ...
      try {
        ...
        // @ts-ignore
        const result = await window.electron.modifyLines(file.path, outputMode === 'new_folder' ? targetPath : undefined, {
          mode: modifyMode,
          type: lineType,
          range: lineRange,
          replacementText: actualReplacementTextRef.current
        });
        ...
        setTotalModifiedLines(prev => prev + result.modifiedLines);
  ...
  const columns = [
      ...
      render: (_: any, record: FileItem) => {
        ...
        return (
          <Tooltip title={record.errorMessage}>
            <Tag color={config.color} style={{ borderRadius: '6px', padding: '2px 10px' }}>
              {config.text}
            </Tag>
          </Tooltip>
        );
    ...
    },
    {
      ...
      render: (_: any, record: FileItem) => (
        <Popconfirm
          title={t('确定要移除此文件吗？')}
          onConfirm={() => handleRemoveFile(record.uid)}
          okText={t('确定')}
          cancelText={t('取消')}
    ...
    <div style={{ height: '100%', padding: '0 24px 32px 24px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: '350px' }}>
              <div style={{ marginBottom: '12px', fontWeight: 500, color: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <EditOutlined style={{ color: '#1890ff' }} /> {t('参数设置')}
              </div>
              <Form layout="horizontal" size="small" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} labelAlign="left">
                <Form.Item label={t('操作模式')} style={{ marginBottom: '12px' }}>
                  <Radio.Group 
                    value={modifyMode} 
                    onChange={e => setModifyMode(e.target.value)} 
                    optionType="button"
                    buttonStyle="solid"
                  >
                    <Radio.Button value="delete">{t('删除行')}</Radio.Button>
                    <Radio.Button value="replace">{t('替换行')}</Radio.Button>
                  </Radio.Group>
                </Form.Item>

                <Form.Item label={t('行范围设置')} style={{ marginBottom: lineType === 'range' ? '12px' : 0 }}>
                  <Radio.Group value={lineType} onChange={e => setLineType(e.target.value)}>
                    <Radio value="first">{t('首行')}</Radio>
                    <Radio value="last">{t('尾行')}</Radio>
                    <Radio value="range">{t('指定范围')}</Radio>
                  </Radio.Group>
                </Form.Item>
                ...
                {lineType === 'range' && (
                  <Form.Item label=" " colon={false} style={{ marginBottom: 0 }}>
                    <Input 
                      placeholder={t('例如: 1-5, 8, 10-')} 
                      value={lineRange} 
                      onChange={e => setLineRange(e.target.value)}
                      suffix={
                        <Tooltip title={t('1-5: 第1到5行; 8: 第8行; 10-: 第10行到最后。逗号隔开。')}>
                ...
                {modifyMode === 'replace' && (
                  <>
                    <Form.Item label={t('内容来源')} style={{ marginTop: '12px', marginBottom: '8px' }}>
                      <Radio.Group value={replaceType} onChange={e => setReplaceType(e.target.value)}>
                    ...
                    <Form.Item label=" " colon={false} style={{ marginBottom: 0 }}>
                      ...
                      <Button type="link" size="small" onClick={handlePreviewReplacement} style={{ padding: 0, marginTop: 4 }}>{t('预览替代内容')}</Button>
                    </Form.Item>
                  </>
                )}
              </Form>
            ...
            <div style={{ flex: 1, minWidth: '300px' }}>
              ...
              <Form layout="horizontal" size="small" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} labelAlign="left">
                <Form.Item label={t('输出方式')} style={{ marginBottom: '12px' }}>
                  <Radio.Group value={outputMode} onChange={e => setOutputMode(e.target.value)} optionType="button" buttonStyle="solid">
                    <Radio.Button value="overwrite">{t('覆盖源文件')}</Radio.Button>
                    <Radio.Button value="new_folder">{t('输出到新文件夹')}</Radio.Button>
                  </Radio.Group>
                </Form.Item>
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ 
            ...
            <br />
            {t('共修改')}{' '}
            <span style={{ color: '#52c41a', fontWeight: 600 }}>{totalModifiedLines}</span>{' '}
            {t('行')}
          </div>
          <Space size="middle">
            <Button 
      ...
      >
        ...
        <Table
          ...
          rowKey="path"
          columns={[
            { title: t('文件名'), dataIndex: 'name', key: 'name', render: (text: string) => <><FileTextOutlined style={{ marginRight: 8 }} />{text}</> },
            { title: t('相对路径'), dataIndex: 'relDir', key: 'relDir', render: (text: string) => text || '.' },
            { 
              title: t('大小'), 
              dataIndex: 'size', 
              key: 'size', 
      ...
      <Modal
        ...
        footer={[<Button key="close" onClick={() => setIsPreviewVisible(false)}>{t('关闭')}</Button>]}
        width={700}
      >
        <div style={{ 
          maxHeight: '400px', 
          ...
          {previewContent || <Text type="secondary">{t('无内容')}</Text>}
        </div>
      </Modal>
    </div>
  );
